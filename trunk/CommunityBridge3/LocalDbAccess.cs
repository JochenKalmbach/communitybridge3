...
namespace CommunityBridge3
{
    ...
    /// </summary>
    /// <remarks>
    /// Infos about AppLocal-Deployment: http://www.ttkalec.com/blog/deploying-net-apps-that-use-ef-and-sql-ce-4-0/
    /// Better Infos about AppLocal-Deployment: http://erikej.blogspot.de/2012/05/private-deployment-of-sql-server.html
    /// </remarks>
                        ...
                        if (File.Exists(fn) == false) // prüfe hier nochmals, da ich nur hier den Mutex besitze!
                        {
                            using (var db =
                              typeof(NewsgroupsEntities).Assembly.GetManifestResourceStream(
                                UserSettings.ProductName + ".Newsgroups.sdf"))
                            {
