...
namespace CommunityBridge3.NNTPServer
{
    public abstract class DataProvider : IDisposable 
    {
      ...
      protected void OnProgressData(string newsgroup, string text)
      {
        ProgressDataEventHandler handler = ProgressData;
        if (handler != null)
          handler(this, new ProgressDataEventArgs { Newsgroup = newsgroup, Text = text});
      }
    }
